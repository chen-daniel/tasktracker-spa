...
function TaskForm(props) {
  ...
  if (props.form.user_id) {
    ...
    return <div style={{padding: "4ex"}}>
      <h2>New Task</h2>
      <FormGroup className="hidden-field">
        <Input className="hidden-field" type="text" name="created_by_id" value={props.form.user_id} onChange={update}>
        </Input>
      </FormGroup>
      <FormGroup>
        <Label for="assigned_to_id">Assign to</Label>
        <Input type="select" name="assigned_to_id" value={props.form.assigned_to_id} onChange={update}>
          { users }
        </Input>
      </FormGroup>
      <FormGroup>
